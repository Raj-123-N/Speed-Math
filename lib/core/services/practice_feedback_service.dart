...
///
/// Keeping feedback behind one service makes Practice responsive without
/// coupling question widgets to platform/audio details.
class PracticeFeedbackService {
  ...
  Future<void> _play(String asset, {Future<void> Function()? haptic}) async {
    if (!_loaded) await initialize();
    if (_vibrationEnabled && haptic != null) {
      await haptic();
    }
    ...
    try {
      ...
      // Audio is enhancement-only. A failed sound must never break Practice.
    }
  }
