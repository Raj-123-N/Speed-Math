...
class TopicLearnScreen extends StatelessWidget {
      ...
      body: ListView(
        ...
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(color: AppColors.primary.withValues(alpha: .08), borderRadius: BorderRadius.circular(18)),
            child: const Text('Learning rule: understand the method, verify one example, then repeat the topic in Practice. Revision is for recall after learning.', style: TextStyle(fontWeight: FontWeight.w700, height: 1.4)),
          ),
        ],
      ),
    );
  ...
  Widget _hero(bool dark) => Container(
    ...
    decoration: BoxDecoration(
      gradient: LinearGradient(colors: [AppColors.primary.withValues(alpha: .18), AppColors.primary.withValues(alpha: .04)]),
      borderRadius: BorderRadius.circular(22),
      border: Border.all(color: AppColors.primary.withValues(alpha: .18)),
    ),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(topic.level.name.toUpperCase(), style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w900, color: AppColors.primary, letterSpacing: 1)),
      const SizedBox(height: 7),
      Text(topic.summary, style: TextStyle(fontSize: 15, color: dark ? Colors.white70 : Colors.black87, height: 1.4)),
    ]),
  );

  Widget _section(String title, List<String> items, IconData icon, bool dark) {
    final children = <Widget>[
      Row(children: [Icon(icon, color: AppColors.primary), const SizedBox(width: 9), Text(title, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 16))]),
