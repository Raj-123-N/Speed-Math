...
class _PracticeSetupScreenState extends State<PracticeSetupScreen> {
  ...
  void initState() {
    ...
    if (widget.category.operation == MathOperation.table) _input = PracticeInputMode.keyboard;
  }
  ...
  Widget build(BuildContext context) {
    ...
    final bg = dark ? AppColors.backgroundDark : const Color(0xFFF3F5F9);
    return Scaffold(
      backgroundColor: bg,
      appBar: AppBar(
      ...
      body: ListView(
        ...
        children: [
          _HeroCard(accent: accent, dark: dark),
          const SizedBox(height: 12),
          if (_isArithmetic) ...[_numberSettings(dark, accent), const SizedBox(height: 12)],
          if (_isTables) ...[_tableSettings(dark, accent), const SizedBox(height: 12)],
          if (!_isTables && !_isArithmetic && !_isRecall) ...[_complexitySettings(dark, accent), const SizedBox(height: 12)],
  ...
  Widget _HeroCard({required Color accent, required bool dark}) => Container(
  ...
  Widget _numberSettings(bool dark, Color accent) => _section(dark, accent, 'Number structure', [
    ...
    _stepper('Number of terms', _terms, 2, 6, (v) => setState(() => _terms = v)),
  ]);

  Widget _tableSettings(bool dark, Color accent) => _section(dark, accent, 'Table drill', [
    _rangeRow('Tables', _tableStart, _tableEnd, (a, b) => setState(() { _tableStart = a; _tableEnd = b; })),
  ...
  Widget _rangeRow(String label, int a, int b, void Function(int,int) onChanged) => ListTile(contentPadding: EdgeInsets.zero, title: Text(label, style: const TextStyle(fontWeight: FontWeight.w700)), subtitle: const Text('1–100 tables'), trailing: Row(mainAxisSize: MainAxisSize.min, children: [_smallField(a, (v) => onChanged(v.clamp(1, b), b)), const Padding(padding: EdgeInsets.symmetric(horizontal: 6), child: Text('to')), _smallField(b, (v) => onChanged(a, v.clamp(a, 100))) ]));
  ...
  void _start() {
    final config = PracticeConfig(category: widget.category, pattern: _pattern, lhsDigits: _lhs, rhsDigits: _rhs, terms: _terms, questions: _questions, complexity: _complexity, timeMode: _timeMode, timeLimitSeconds: _seconds, inputMode: _input, autoSubmit: _autoSubmit, quickSubmit: _quickSubmit, tableStart: _tableStart, tableEnd: _tableEnd, multiplierMax: _multiplier, tableOrder: _tableOrder, shuffleSequential: _shuffleSequential);
