...
class _PracticeSessionScreenState extends State<PracticeSessionScreen> {
  ...
  void _submit(String raw) {
    if (_locked || raw.trim().isEmpty) return;
    ...
    Future.delayed(Duration(milliseconds: widget.config.quickSubmit ? 100 : 260), () {
      if (!mounted) return;
  ...
  void _finish() {
    ...
    Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => PracticeResultScreen(result: PracticeResult(total: _index + (_locked ? 1 : 0), correct: _correct, wrong: _wrong, elapsed: elapsed), categoryName: widget.config.category.name)));
  }
    ...
    return Scaffold(
      ...
      body: SafeArea(child: Column(children: [
        ...
        Expanded(child: ListView(padding: const EdgeInsets.fromLTRB(16, 26, 16, 24), children: [
          Center(child: Text('QUESTION ${_index + 1}', style: TextStyle(fontWeight: FontWeight.w900, letterSpacing: 1.4, color: accent))),
          ...
          if (widget.config.inputMode == PracticeInputMode.mcq) _buildMcq(accent, dark) else _buildKeyboard(accent, dark),
          const SizedBox(height: 18),
          if (_locked) Center(child: Text(_answer.text.trim() == _question.answer ? 'Correct ✓' : 'Answer: ${_question.answer}', style: TextStyle(fontWeight: FontWeight.w800, color: _answer.text.trim() == _question.answer ? Colors.green : Colors.red))),
        ])),
      ])),
    );
  }
  ...
  Widget _buildKeyboard(Color accent, bool dark) => Column(children: [
    TextField(controller: _answer, focusNode: _focus, autofocus: true, keyboardType: TextInputType.numberWithOptions(decimal: true, signed: true), textInputAction: TextInputAction.done, onSubmitted: _submit, onChanged: (v) { if (widget.config.autoSubmit && v.trim().isNotEmpty && v.trim().replaceAll(' ', '') == _question.answer.replaceAll(' ', '')) _submit(v); }, decoration: InputDecoration(labelText: _question.inputHint, hintText: 'Type answer…', prefixIcon: Icon(Icons.keyboard_alt_outlined, color: accent), suffixIcon: IconButton(onPressed: () => _submit(_answer.text), icon: Icon(Icons.bolt_rounded, color: accent)), filled: true, fillColor: dark ? AppColors.cardDark : Colors.white, border: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: BorderSide(color: accent.withValues(alpha: .3))), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: BorderSide(color: accent, width: 2)) ),
